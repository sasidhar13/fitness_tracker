...
from FrequencyAbstractions import FourierTransformation
from sklearn.cluster import KMeans
# --------------------------------------------------------------
# Load data
# --------------------------------------------------------------
df = pd.read_pickle('../../data/interim/outlier_removed_chauvenet.pkl')
...
duration=df[df['set']==2].index[-1]-df[df['set']==2].index[1]

for set in df['set'].unique():
    start = df[df['set']==set].index[1]
...
# 5 reps is for heavy set, 10 for medium
...
ax[1].plot(subset['acc_y_lowpass'].reset_index(drop=True),label="lowpass data")
...
for s in df_freq['set'].unique():
    print(f'Fourier trnsformation for {s}')
...
for l in df_cluster["label"].unique():
    ...
    ax.scatter(subset["acc_x"],subset['acc_y'],label=l)
